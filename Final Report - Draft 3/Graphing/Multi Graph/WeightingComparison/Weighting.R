mydata <- read.csv("Multi Graph/WeightingComparison/Weighting.csv")

library(ggplot2)

time <- ggplot(mydata, aes(x=factor(Weighting), Total.Time))

time + geom_boxplot(aes(fill = factor(BenchMark))) + ggtitle("Comparison between\n Weighting and No Weighting") + 
     theme(plot.title = element_text(lineheight=.8, face="bold")) +
     xlab("Weighting") + ylab("Total Time Taken")
     
     

ggplot(data=mydata, aes(x=Weighting, y=Output.Comparisons, group=BenchMark, shape=BenchMark, colour=BenchMark)) + 
    ...
    xlab("Weighting") + ylab("Number of redundant tests") + # Set axis labels
    ggtitle("Comparison between Weighting and No Weighting\n in relation to Redundant Tests Identified") +     # Set title
